...
pub fn packSize(data: anytype) comptime_int {
...
pub fn pack(pack_buf: []u8, packIndex: *usize, data: anytype) void {
    ...
    switch (@typeInfo(@TypeOf(data))) {
        ...
        .Float => |f| {
            if (f.bits == 32) {
                ...
                packIndex.* += 1;
                const float_bytes = toBytes(@byteSwap(u32, @bitCast(u32, data)));
                copy(u8, pack_buf[packIndex.*..pack_buf.len], &float_bytes);
                packIndex.* += float_bytes.len;
            } else if (f.bits == 64) {
                pack_buf[packIndex.*] = 0xcb;
                packIndex.* += 1;
                const float_bytes = toBytes(@byteSwap(u64, @bitCast(u64, data)));
                copy(u8, pack_buf[packIndex.*..pack_buf.len], &float_bytes);
                packIndex.* += float_bytes.len;
            } else {
